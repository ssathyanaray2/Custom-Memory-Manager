...
{
	if (bitmap == NULL || size <= 0 || (val != 0 && val != 1)) {
    ...
    for (int byte = 0; byte < size; byte++) {
        ...
        for (int bit = 0; bit < 8; bit++) {
            int current_bit = (current_byte >> bit) & 1;
...
int bitmap_set_bit(unsigned char * bitmap, int size, int target_pos)
{
    ...
    int byte_index = target_pos / 8;

    bitmap[byte_index] = bitmap[byte_index] | (1 << (target_pos % 8));
...
int bitmap_bit_is_set(unsigned char * bitmap, int size, int pos)
{
   if (bitmap == NULL || size <= 0 || pos < 0 || pos >= size * 8) {
    ...
    return (bitmap[byte_index] >> (pos % 8)) & 1;
}
